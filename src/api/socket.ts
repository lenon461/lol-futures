class Socket {
    ...
    addEvent(eventName, callback?) {
        ...
        if(!callback) callback = () => console.log(eventName + " callback is not registered")
    ...
    on(eventName, callback?) {
        console.log("on")
        if(!Object.keys(this.events).find(event => event == eventName)) this.addEvent(eventName, callback)
        console.log("")
    }
    emit(event, args) {
